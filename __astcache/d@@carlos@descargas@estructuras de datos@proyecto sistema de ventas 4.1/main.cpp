...
void __fastcall TForm1::PreCargar(){
   ...
   if (!carritoA->Vacia()) {
	 ...
	 while (!aux->Vacia()){
	   ...
	   AgregarProducto(car,codigo,nomb.TrimLeft(),cantidad,precio,tot);
...
void __fastcall TForm1::Button5Click(TObject *Sender)
{
	if (!carritoA->Vacia()) {
	 ...
	 AnsiString f = FormatDateTime("dd/mm/yyyy", DateTimePicker1->DateTime);
     AnsiString nom;
...
{  int numvent = StrToInt(InputBox("Buscar venta:","Numero de venta: ",""));
   ...
   buscar_venta(numvent,codcl,mont,fecha);
	if (fecha!="") {
	 ...
	 MessageDlg("No se encontró la venta.", mtError, TMsgDlgButtons() << mbOK, 0);
}
//---------------------------------------------------------------------------

void __fastcall TForm1::Button6Click(TObject *Sender)
{
  int respuesta = MessageDlg("¿Estás seguro que quieres cancelar la venta?", mtConfirmation,
...
{  int cod = StrToInt(InputBox("Buscar","Codigo de cliente: ",""));
   ...
   for (int i=0; i < cliente->longitud(); i++) {
	 ...
	 if (cod == c ) {
		  no=s;
		  int respuesta = MessageDlg("¿Estás seguro que quieres eliminar a "+s+" de la lista de clientes?", mtConfirmation,
								   TMsgDlgButtons() << mbOK << mbCancel, 0);
		 if (respuesta == mrOk)	{
...
{
    // Cambiar el color de fondo de las filas alternadamente
    if (Item->Index % 2 == 0) {
		Sender->Canvas->Brush->Color = clSilver; // Color para filas pares
    } else {
		Sender->Canvas->Brush->Color = clSkyBlue;   // Color para filas impares
    }

    // Cambiar el color del texto si lo deseas
    // Sender->Canvas->Font->Color = clBlack;    // Color del texto

    // Permitir que el sistema dibuje el resto de la fila (texto, iconos, etc.)
	DefaultDraw = true;
...
{
	 codcliente->Text = DateTimePicker1-> Date;
}
//---------------------------------------------------------------------------
