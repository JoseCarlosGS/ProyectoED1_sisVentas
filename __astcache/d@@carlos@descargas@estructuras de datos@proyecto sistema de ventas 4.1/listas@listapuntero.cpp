...
void ListaP::recupera(NodoL* p, int& cod ,AnsiString& inf) {
	if (this->vacia()) {
		cout << "LISTA VACIA\n";
	}
	else {
...
void ListaP::inserta(NodoL* p, int e, AnsiString n) {
	NodoL* x = new NodoL;
	if (x != NULL) {
		x->info.codigo = e;
		x->info.nombre = n;
...
void ListaP::inserta_primero(int e, AnsiString n)
{
NodoL* x= new NodoL;
if(x!=NULL)
{
...
void ListaP::inserta_ultimo(int e, AnsiString n) {
	NodoL* x = new NodoL;
if(x!=NULL)
{
	x->info.codigo=e;
...
void ListaP::modifica(NodoL* p, int e, AnsiString n) {
	if (vacia()) {
		cout << "LISTA VACIA\n";
	}
	else {
